...
The repository includes workflows in `codemagic.yaml`:
- `somcep-ios-testflight`: Builds a signed SOMCEP IPA with bundle identifier `com.itechevent.somcep2026` and uploads it to App Store Connect.
- `agpc-ios-testflight`: Builds a signed AGPC IPA with bundle identifier `com.itechevent.agpc2026` and uploads it to App Store Connect.

Before the first SOMCEP build:

1. Register `com.itechevent.somcep2026` in Apple Developer.
2. Create the new `SOMCEP Events` (or `SOMCEP 2026`) app record in App Store Connect.
3. Add or select the App Store Connect integration in Codemagic.
4. Run the `SOMCEP 2026 - iOS TestFlight` workflow from `main`.

The workflow uploads to App Store Connect TestFlight without automatically submitting the app for App Review.
